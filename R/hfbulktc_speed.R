...
z0t<-7.5*10^(-5)
ctnhf<-kappa_val/log(zt/z0t)

z0q<-z0t
cqnhf<-kappa_val/log(zq/z0q)
...
while (cvrgnce){ #modified by TSA
    ...
    cdnhf<-kappa_val/(log(zr/z0)-psi(zetu, "utc"))
    U_star<-cdnhf*S
    ...
    cthf<-kappa_val/(log(zt/z0t)-psi(zett, "ttc"))
    cqhf<-kappa_val/(log(zq/z0q)-psi(zetq, "ttc"))
    ...
    Ws<-U_star*(-CVB_depth/(kappa_val*L))^onethird
